...
async function restoreStock() {
    const client = new MongoClient(process.env.MONGODB_URL || 'mongodb://localhost:27017/fishhunt');
    try {
        ...
        if (stockRestorations.length === 0) {
            console.log('No stock restorations defined. Please edit the script with your specific data.');
            return;
        }
            ...
            if (size || color || ageGroup) {
                ...
                if (variantIndex >= 0) {
                    await productsCollection.updateOne({
                        ...
                        $inc: {
                            [`variants.${variantIndex}.stock`]: qty
                        }
                    });
                    console.log(`Restored ${qty} units to variant ${size}/${color}/${ageGroup} of product ${product.name}`);
                } else {
...
//# sourceMappingURL=restore-stock.js.map