...
let OrdersService = class OrdersService {
    async create(orderAttrs, userId, externalOrderId) {
        const { orderItems, shippingDetails, paymentMethod, itemsPrice, taxPrice, shippingPrice, totalPrice } = orderAttrs;
        if (orderItems && orderItems.length < 1) throw new _common.BadRequestException('No order items received.');
        ...
        try {
            return await session.withTransaction(async ()=>{
                ...
                for (const item of orderItems){
                    ...
                    if (!product) {
                        throw new _common.NotFoundException(`Product with ID ${item.productId} not found`);
                    }
                    ...
                    if (product.variants && product.variants.length > 0 && (item.size || item.color || item.ageGroup)) {
                        ...
                        if (variantIndex === -1) {
                            throw new _common.BadRequestException(`Variant not found for product ${product.name} (${item.size}/${item.color}/${item.ageGroup})`);
                        }
                        if (product.variants[variantIndex].stock < item.qty) {
                            throw new _common.BadRequestException(`Not enough stock for product ${product.name} variant (${item.size}/${item.color}/${item.ageGroup}). Available: ${product.variants[variantIndex].stock}, Requested: ${item.qty}`);
                        }
                        ...
                        if (product.countInStock < item.qty) {
                            throw new _common.BadRequestException(`Not enough stock for product ${product.name}. Available: ${product.countInStock}, Requested: ${item.qty}`);
                        }
                ...
                const createdOrder = await this.orderModel.create([
                    {
                        ...
                        totalPrice,
                        externalOrderId,
                        stockReservationExpires: new Date(Date.now() + 10 * 60 * 1000)
                    }
                ], {
                    session
                });
    ...
    async findAll() {
        ...
        const orders = await this.orderModel.find().populate('user', 'name email').sort({
            createdAt: -1
        });
        return orders;
    }
    async findById(id) {
        if (!_mongoose1.Types.ObjectId.isValid(id)) throw new _common.BadRequestException('Invalid order ID.');
        const order = await this.orderModel.findById(id).populate('user', 'name email');
        if (!order) throw new _common.NotFoundException('No order with given ID.');
        return order;
    }
    async updatePaid(id, paymentResult) {
        if (!_mongoose1.Types.ObjectId.isValid(id)) throw new _common.BadRequestException('Invalid order ID.');
        const order = await this.orderModel.findById(id);
        if (!order) throw new _common.NotFoundException('No order with given ID.');
        // Check if order is already paid
        if (order.isPaid) {
            throw new _common.BadRequestException('Order is already paid.');
        }
        // შეამოწმოს თუ შეკვეთა cancelled სტატუსშია, მაშინ არ დაუშვას გადახდა
        if (order.status === 'cancelled') {
            throw new _common.BadRequestException('Cannot pay for cancelled order. Please create a new order.');
        }
        ...
        const isExpired = order.stockReservationExpires && new Date() > order.stockReservationExpires;
        if (isExpired) {
            // თუ რეზერვაცია ამოიწურა, მაგრამ გადახდა მოვიდა, მაინც შევცვალოთ სტატუსი გადახდილში
            this.logger?.log(`Payment received for expired reservation order ${id}. Processing payment anyway.`);
        }
        ...
        if (!isExpired) {
            ...
            for (const item of order.orderItems){
                ...
                if (!product) {
                    throw new _common.NotFoundException(`Product ${item.name} is no longer available.`);
                }
                // Check stock availability
                let stockAvailable = false;
                if (product.variants && product.variants.length > 0 && (item.size || item.color || item.ageGroup)) {
                    // Check variant stock
                    const variant = product.variants.find((v)=>v.size === item.size && v.color === item.color && v.ageGroup === item.ageGroup);
                ...
                if (!stockAvailable) {
                    throw new _common.BadRequestException(`Product "${item.name}" ${item.size ? `(${item.size}/${item.color}/${item.ageGroup})` : ''} is no longer available. Stock has been exhausted.`);
                }
            }
        }
        order.isPaid = true;
    ...
    async findByExternalOrderId(externalOrderId) {
        const order = await this.orderModel.findOne({
            externalOrderId
        });
        if (!order) {
            throw new _common.NotFoundException(`Order with external ID ${externalOrderId} not found`);
        }
        return order;
    }
    async updateOrderByExternalId(externalOrderId, paymentResult) {
        ...
        console.log(`Found order: ${order._id}, current isPaid: ${order.isPaid}, current status: ${order.status}`);
        // Check if order is already paid
        if (order.isPaid) {
        ...
        if (isExpired) {
            // თუ რეზერვაცია ამოიწურა, მაგრამ გადახდა მოვიდა, მაინც შევცვალოთ სტატუსი გადახდილში
            this.logger.log(`Payment received for expired reservation order ${externalOrderId}. Processing payment anyway.`);
        }
        // შევამოწმოთ სტოკი მხოლოდ იმ შემთხვევაში თუ რეზერვაცია არ ამოიწურა
        if (!isExpired) {
            ...
            for (const item of order.orderItems){
                ...
                if (!stockValid) {
                    throw new _common.BadRequestException(`Product "${item.name}" stock integrity issue.`);
                }
            }
        }
        ...
        const updatedOrder = await order.save();
        console.log(`Order ${externalOrderId} successfully updated. New isPaid: ${updatedOrder.isPaid}, new status: ${updatedOrder.status}`);
        return updatedOrder;
    }
    /**
   ...
   */ async refundStock(orderId) {
        ...
        if (!order) {
            throw new _common.NotFoundException(`Order with ID ${orderId} not found`);
        }
        const session = await this.connection.startSession();
        try {
            await session.withTransaction(async ()=>{
                for (const item of order.orderItems){
                    const product = await this.productModel.findById(item.productId).session(session);
                    if (!product) {
                        console.warn(`Product with ID ${item.productId} not found during stock refund`);
                        continue;
                    }
                    // Refund stock
                    if (product.variants && product.variants.length > 0 && (item.size || item.color || item.ageGroup)) {
    ...
    async cancelOrder(id, reason) {
        ...
        if (order.status === 'cancelled') {
            throw new _common.BadRequestException('Order is already cancelled.');
        }
        // Check if order is already paid - paid orders cannot be cancelled automatically
        if (order.isPaid) {
            throw new _common.BadRequestException('Cannot cancel paid order automatically. Please contact support.');
        }
        const session = await this.connection.startSession();
        try {
            return await session.withTransaction(async ()=>{
    ...
    async findUserOrdersByStatus(userId, status) {
        ...
        // Sort by createdAt in descending order (newest first)
        const orders = await this.orderModel.find(query).sort({
            createdAt: -1
        });
        return orders;
    }
   ...
   */ async refundStockForOrder(order, session) {
    ...
    async sendSuccessNotification(orderId) {
        try {
            const order = await this.orderModel.findById(orderId).populate('user', 'name email').populate('orderItems.product', 'name user');
            if (!order) {
            ...
            const orderData = {
                ...
                items: order.orderItems.map((item)=>({
                        ...
                        quantity: item.qty,
                        price: item.price,
                        sellerEmail: item.product.user?.email
                    })),
    ...
    async sendFailureNotification(orderId) {
        try {
    ...
    constructor(orderModel, productModel, userModel, productsService, connection, emailService){
...
OrdersService = _ts_decorate([
    ...
    _ts_param(2, (0, _mongoose.InjectModel)(_userschema.User.name)),
    _ts_param(4, (0, _mongoose.InjectConnection)()),
