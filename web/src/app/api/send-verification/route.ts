...
export async function POST(
  ...
  try {
    await database.addEntry({ email, code: verificationCode, expiration: Date.now() + EXPIRATION });
    // Send the email
    ...
    if (error instanceof Error) {
      ...
      return new Response(JSON.stringify({ success: false, message: "Failed to send email", error: error.message, }), { status: 500, headers: { "Content-Type": "application/json" } });
    } else {
