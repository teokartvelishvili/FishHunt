...
export async function POST(
  ...
  if (verificationEntry.expiration < Date.now()) {
    return new Response(JSON.stringify({ success: false, message: "Verification code expired" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }

  if (verificationEntry.code !== code) {
    return new Response(JSON.stringify({ success: false, message: "Invalid verification code" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
